<!doctype html>
<html lang = "en">
<head>
<meta charset="utf-8">
<title>Alvsroa</title>
<meta name="keywords" content="web development,computer tweaking,SEO, online marketing,web design, computer troubleshooting">
<meta name="description" content="A Passionate IT doer and Web Enthusiast">
<meta http-equiv="X-UA-Compatible" content="IE=Edge, chrome=1">
<meta name="viewport" content="width=device-width, intial-scale=1.0">
<meta name="author" content="Alvin S. Roasol">
<link href="css/custom.css" rel="stylesheet">
<link href="img/favicon.ico" rel="shortcut icon">
<link href="css/bootstrap.css" rel="stylesheet">
<link href="css/animate.css" rel="stylesheet">
<script src="js/jquery-1.9.1.js" type="text/javascript"></script>
<script src="js/bootstrap.min.js" type="text/javascript"></script>
<script src="js/walkway.js" type="text/javascript"></script>
<script src="js/wow.min.js" type="text/javascript"></script>
<script src="js/custom.js" type="text/javascript"></script>
	<!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->
</head>
<body data-spy="scroll" data-target=".navbar">
<div class="wrapper">
  <nav class="navbar navbar-default navbar-fixed-top navbar-inverse" role="navigation">
  	<div class="container">
        <div class="col-lg-1">
        </div>
        <div class="col-lg-8">
            <div class="navbar-header">
              <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#target-collapse">
              <span class="sr-only">toggle navigation</span>
              <span class="icon-bar"></span>
              <span class="icon-bar"></span>
              <span class="icon-bar"></span>
              </button>
            <div class="collapse navbar-collapse" id="target-collapse">
              <ul class="nav navbar-nav nav-left">
                  <li class="active"><a href="#view-home">Home</a></li>	
                  <li><a href="#view-about">About</a></li>	
                  <li><a href="#view-contact">Contact</a></li>	
              </ul>
            </div> <!---end-collapse--->
            </div> <!---end-navbar header--->
          </div> <!---end-col-8--->
          <div class="col-lg-3">
          </div>
      </div> <!---end-container--->
  </nav>
  <div id="view-home" class=" container-fluid floor">
  <div  class="distance-uppermost">
    <div class="col-lg-2 col-md-1 hidden-xs">           	
    </div>
    <div class="col-lg-8 col-md-10 col-sm-10 col-xs-12">
      <hgroup>
        <h1 id = "home" class=" distance-top-reset text-center text-head"><strong>Hi I'm Alvs</strong></h1>
        <h2 class=" distance-top-reset text-center text-head-subtext">a Passionate IT doer and Web Enthusiast</h2>
      </hgroup>
      <img src="img/lamp-table.png" alt="table" class="img-responsive table-align">
    </div>
    <div class="col-lg-2 col-md-1 col-sm-1 hidden-xs">
    	<ul class=" list-unstyled pull-right social-connections">
        	<li><img src="img/facebook.png" alt="facebook icon" height="52" width="52"></li>
            <li><img src="img/google+.png" alt="google+ icon"  height="52" width="52"></li>
            <li><img src="img/github-icon.png" alt="github icon"  height="52" width="52"></li> 
            <li><img src="img/twitter.png" alt="twitter icon"  height="52" width="52"></li>
            <li><img src="img/dribbble.png" alt="dribbble icon"  height="52" width="52"></li>
        </ul>
    </div>
  </div> <!---end-container-fluid-upper--->
  </div>
  <div id="view-about" class="container-fluid"><!--start-about-me-->
  	<div class="container">
    	<div class="col-lg-12">
        	<h4 class="text-center content-header"><strong>About me</strong></h4>
        </div>
    	<div class="col-lg-1 col-md-1 col-sm-1 hidden-xs">
        </div>
    	<div class="col-lg-3 col-md-3 col-sm-3">
        	<img src="img/profile-pic.png" title="profile picture" class="img-rounded img-responsive img-center">
        </div>
        <div class="col-lg-7 col-md-7 col-sm-7">
			<article>
				<p class="about-text">Hello it's me and yes that is my face...</p>
				<p class="about-text">It’s my pleasure to meet you here at my workplace by the way I’m great fan of Web Technology and 
				everything about Computer, that’s why I love Web development and do Computer tweaking. If you find these things I do 
				are interesting and would like to share your awesome Ideas with me, you may tickle me an Email.
				</p>
				<p class="about-text"><q>I make awesome things happen</q></p> 
			</article>
        </div>
        <div class="col-lg-12 separating-div"></div>
         <div class="container skills-distance">
          <div class="col-lg-4 col-md-4 col-sm-4 clearfix"><!--start-skills-icon-->
              <figure title = "Computer Technology">
                <img class="img-responsive img-center" src="img/computer.png" alt="computer desktop icon">
                <figcaption class="caption-img">Computer<br /> Technology</figcaption>
              </figure>
          </div>
          <div class="col-lg-4 col-md-4 col-sm-4 clearfix">
              <figure title = "SEO and marketing">
                <img class="img-responsive img-center" src="img/SEO marketing.png" alt="SEO and Marketing icon">
                <figcaption class="caption-img">SEO and<br /> Marketing</figcaption>
              </figure>
          </div>
          <div class="col-lg-4 col-md-4 col-sm-4 clearfix">
              <figure title = "Web Design and Development">
                <img class="img-responsive img-center" src="img/design and development.png" alt=" Web Design and Development">
                <figcaption class="caption-img">Web Design and<br />Development</figcaption>
              </figure>
          </div><!--end-skills-icon-->
        </div>
    </div>
  </div><!--end-about-me-->
  <div  class="container contact-bg"><!--start-contact-me-->
  	<div class="col-lg-12">
    	<h4 id="view-contact" class="text-center contact-head"><strong>Contact me</strong></h4>
    </div> 
    <div class=" col-lg-12">
    	<div class="row">
        	<form role="form" action="sendmails.php" method="POST">
            	<div class="col-lg-5 col-md-5 col-sm-5"><!--start-input-field-->
                    <div class="form-group">
                      <label for="name">Name</label>
                      <input id="name" type="text" class="form-control" placeholder="Your name" required autofocus name="fname">
                    </div>
                    <div class="form-group">
                      <label for="email">Email</label>
                      <input id="email" type="email" class="form-control" placeholder="Your Email" required name="email">
                    </div>
                    <div class="form-group">
                      <label for="interest">field of Interest</label>
                      <select id="interest" name="interest" class="form-control">
                        <option value="computer technology">Computer Technology</option>
                         <option value="computer technology">SEO and Marketing</option>
                         <option value="computer technology">Web Design and Development</option>
                      </select>
                    </div>
                </div><!--end-input-field-->
                <div class="col-lg-7 col-md-7 col-sm-7"><!--start-comments-field-->
                	<div class="form-group">
                    	<label for="comments">Comments</label>
                    	<textarea id="comments" class="form-control" rows="5"  placeholder="Enter your Comments" required name="comments">
                        </textarea><br />
                        <button class="btn btn-primary submit-btn">Submit</button>
                    </div>	
                </div><!--start-comments-field-->
            </form>
            <div class="status-email">
            	
            </div>
        </div>
    </div> 
  </div><!--end-contact-me-->
  <div id="device" class="container-fluid devices-bg">
  	<div class="container">
    	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
            <h5 class="devices-tagline response-h5">I'm working on these<br/><span>Devices</span></h5>
        </div>
        <div id ="deviceLoad">
		</div>
    </div>
  </div>
  <footer class="container-fluid foo-last">
  	<div class="container">
    	<div class="col-lg-5 col-md-5 col-sm-5">
          <p class="foo-distance response-footer-text">&copy;Copyright Reserved 2015</p>
      	</div>
        <div class="col-lg-7 col-md-7 col-sm-7">
          <p class=" foo-distance response-footer-text pull-right">You may also send Inquiries @ &nbsp;
          <a href="mailto:alvsroa@outlook.com" target="_blank">alvsroa@outlook.com</a></p>
      	</div>
    </div>
  </footer>
</div>
</body>
</html>
